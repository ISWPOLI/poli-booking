...
public class InformacionPorDefecto implements ApplicationListener<ContextRefreshedEvent> {
	...
	public void onApplicationEvent(ContextRefreshedEvent cre) {
		try {
			...
			if (INSERTAR_INFO_POR_DEFECTO) {
				...
				crearBloquesDeCanchasPorDefecto();
				crearBloquesDeGimnasioPorDefecto();
			}
	...
	private void crearCubiculosPorDefecto() {
		...
		TipoEspacioDTO tipoCubiculoEstudio = tipoEspacioService
				.buscarTipoEspacioPorNombre(TipoEspacioDTO.CUBICULO_ESTUDIO);
	...
	private void crearBloquesDeCubiculosPorDefecto() {
		...
		List<EspacioDTO> cubiculosVideo = cubiculoService.getCubiculosVideo();
		for (EspacioDTO cubiculoV : cubiculosVideo) {
			bloqueService.generarBloques(cubiculoV, DIAS_BLOQUES_POR_DEFECTO);
		}
	}

	private void crearBloquesDeCanchasPorDefecto() {
		logger.info("Creando bloques de canchas por defecto");
		List<EspacioDTO> canchasF = canchaService.getCanchaFutbol();
		for (EspacioDTO cancha : canchasF) {
			bloqueService.generarBloques(cancha, DIAS_BLOQUES_POR_DEFECTO);
		}

		List<EspacioDTO> canchasT = canchaService.getCanchaTenis();
		for (EspacioDTO cancha : canchasT) {
			bloqueService.generarBloques(cancha, DIAS_BLOQUES_POR_DEFECTO);
		}

		List<EspacioDTO> canchasM = canchaService.getCanchaMultiple();
		for (EspacioDTO canchas : canchasM) {
			bloqueService.generarBloques(canchas, DIAS_BLOQUES_POR_DEFECTO);
		}
	}

	private void crearBloquesDeGimnasioPorDefecto() {
		logger.info("Creando bloques de gimnasio por defecto");
		List<EspacioDTO> Gimnasio = gimnasioService.getGimnasio();
		for (EspacioDTO gimnasio : Gimnasio) {
			bloqueService.generarBloques(gimnasio, DIAS_BLOQUES_POR_DEFECTO);
		}
	}
