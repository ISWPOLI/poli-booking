...
module.exports = function(grunt) {
	...
	grunt
			.initConfig({
				...
				watch : {
					...
					livereload : {
						options : {
							livereload : grunt.option('livereloadport')
									|| LIVERELOAD_PORT
						},
						files : [
								'<%= yeoman.app %>/*.html',
				...
				connect : {
					...
					test : {
						options : {
							port : 9001,
							middleware : function(connect) {
								return [ mountFolder(connect, 'test'),
										lrSnippet,
										mountFolder(connect, '.tmp'),
										mountFolder(connect, yeomanConfig.app) ];
							}
				...
				requirejs : {
					dist : {
						...
						options : {
							...
							mainConfigFile : '<%= yeoman.app %>/scripts/main.js', // contains
																					// path
																					// specifications
																					// and
																					// nothing
	...
	grunt.registerTask('serve', function(target) {
		...
		if (target === 'test') {
			return grunt.task.run([ 'clean:server', 'createDefaultTemplate',
					'jst', 'connect:test', 'open:test', 'watch' ]);
		}

		grunt.task.run([ 'clean:server', 'createDefaultTemplate', 'jst',
				'connect:livereload', 'open:server', 'watch' ]);
	});

	grunt.registerTask('test', function(isConnected) {
		isConnected = Boolean(isConnected);
		var testTasks = [ 'clean:server', 'createDefaultTemplate', 'jst',
				'connect:test', 'mocha' ];
	...
	grunt.registerTask('build', [ 'clean:dist', 'createDefaultTemplate', 'jst',
			'useminPrepare', 'imagemin', 'htmlmin', 'concat', 'cssmin',
			'requirejs', 'uglify', 'copy', 'rev', 'usemin' ]);
